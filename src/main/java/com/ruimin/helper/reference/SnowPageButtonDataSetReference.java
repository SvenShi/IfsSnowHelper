...
public class SnowPageButtonDataSetReference extends PsiReferenceBase<XmlAttributeValue> implements
    ...
    public ResolveResult @NotNull [] multiResolve(boolean incompleteCode) {
        ...
        if (StringUtils.isNotBlank(dataSetId)) {
            List<XmlTag> dataSetList = SnowPageUtils.getDataSetTag(((JspFile) myElement.getContainingFile()));
            ArrayList<ResolveResult> resolveResults = new ArrayList<>();
            for (XmlTag datasetTag : dataSetList) {
                XmlAttribute attribute = datasetTag.getAttribute(SnowPageConstants.DTST_ATTR_NAME_ID);
                if (attribute != null) {

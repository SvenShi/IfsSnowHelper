 ...
 * @description
 */
public class SnowPageJsQueryReference extends PsiReferenceBase<JSReferenceExpression> implements
     ...
     */
    public SnowPageJsQueryReference(@NotNull JSReferenceExpression element) {
    ...
    public ResolveResult @NotNull [] multiResolve(boolean incompleteCode) {
        ...
        if (containingFile != null) {
            List<XmlTag> queryTags = SnowPageUtils.findAllTagInFile((XmlFile) containingFile,
                SnowPageConstants.QUERY_TAG_NAME);
            ArrayList<ResolveResult> resolveResults = new ArrayList<>();
            String text = myElement.getText();
            for (XmlTag xmlTag : queryTags) {
                XmlAttribute attribute = xmlTag.getAttribute(SnowPageConstants.ATTR_NAME_DATASET);
                if (attribute != null) {
                    XmlAttributeValue valueElement = attribute.getValueElement();
                    if (valueElement != null) {
                        String value = valueElement.getValue();
                        if (StringUtils.isNotBlank(value)) {
                            String dataSetExpression = value + SnowPageConstants.QUERY_EXPRESSION_SUFFIX;
